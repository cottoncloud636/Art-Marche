...
import { useParams } from 'react-router-dom'; //a hook to hook the parameter, S.A. id of the list

export default function EditArtList() {
      const navigate = useNavigate(); //initialze the navigate
      const params = useParams();//initialize the params
    ...
    //   console.log(files); //for test purpose: to see if files state gets kept after I chose the images
      ...
      const [submitLoading, setSubmitLoading] = useState(false);
            //use useEffect to get this art list info after page is loaded. It is not directly related to UI, but it
      //is a side-effect I want to have apart from the main UI
      useEffect( ()=> { //it is asynchronous cuz fetching list info takes time, but in useEffect we can't directly
        //use async, hence we need to create a function inside the useEffect and call it
        //below to achieve that
        const fetchListInfo = async ()=>{
            const listId = params.listId; //params.listId retrieves the value of listId from the URL parameters
                        //this listId came from the App.jsx: path='/edit-art/:listId', and
                        //this listId came from the <link> in edit button in Profile.jsx
            // console.log(listId); -- for test purpose: to see if I successfully retrieve the list id from URL parameters
            const res = await fetch(`/api/listing/getlist/${listId}`);
            const data = await res.json();
            if (data.success===false) {
                console.log(data.message); //if I use setSubmitError, msg will display on screen if there is an error occur
                return;
            }
            setFormInfo(data); //initialize the entire form using that specific list's info, because once user
                 //click "edit" btn and goes to edit page, it needs to display that list's original info
        };
        fetchListInfo();
        },[]);
    ...
    //   };
      ...
      const handleFormSubmit = async (event)=>{
        ...
        try {
            ...
            const res = await fetch (`/api/listing/editlist/${params.listId}`, { //addr prefix came from index.js
                                //fetch this endpoint(a workspace from backend) and tell it what I want to post
                method: 'POST',
      ...
      return (
        <div>
            <div className='p-4 max-w-4xl mx-auto'> {/* mx-auto -- brings the content to the center of the page */}
                <h1 className='font-fatface text-5xl text-center mt-5'>Edit your art list below</h1>
                <form onSubmit={handleFormSubmit} className='flex flex-col sm:flex-row mt-8'>
                    ...
                    <div className='flex flex-col ml-6'>
                        ...
                        <button disabled={submitLoading || loading}
                            ...
                            >{submitLoading? 'One moment...' : 'Update it now'}
                        </button>
